...
public:
  ...
  // column_index_num_byte_;
  // value_index_num_byte_;
  // num_init_pairs_, num_double_values_, num_dict_nodes
  void printSummary(){
    cout<<"Summary of Prefix Tree for a Mini-batch"<<endl;

    cout<<"num_distinct_pairs: "<<num_init_pairs_<<"   num_distinct_values: "<<num_double_values_<<"  num_dict_nodes: "<<num_dict_nodes_<<endl;
    
    // int8_t is treated as a char by the stream. So i cannot get the correct output.
    cout<<"code_num_byte: "<< static_cast<int>(code_num_byte_)<<"  column_index_num_byte: "<< static_cast<int>(column_index_num_byte_)<<"  value_index_num_byte: "<< static_cast<int>(value_index_num_byte_)<<endl;
  }

  string getSummary(){
    string s1="Summary of Prefix Tree for a Mini-batch\n";

    string s2="num_distinct_pairs: "+to_string(num_init_pairs_)+"   num_distinct_values: "+to_string(num_double_values_)+"  num_dict_nodes: "+to_string(num_dict_nodes_)+"\n";

    string s3="code_num_byte: "+to_string(static_cast<int>(code_num_byte_))+"  column_index_num_byte: "+to_string(static_cast<int>(column_index_num_byte_))+"  value_index_num_byte: "+to_string(static_cast<int>(value_index_num_byte_))+"\n";
    s1=s1+s2+s3;
    return s1;
  }
...
private:
  ...
  int8_t code_num_byte_=0;
  int8_t column_index_num_byte_=0;
  int8_t value_index_num_byte_=0;
  mutable bool light_decompressed_ = false;
  // The number of init pairs in the compressed matrix.
  // num_init_pairs_, num_double_values_, num_dict_nodes
  int num_init_pairs_ = 0;
