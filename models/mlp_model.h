...
public:
  ...
  // support two kinds of activation function here, sounds great
  ...
  // this is the layer output rather than the final model output
  ...
  // check the backpropagation part tomorrow
...
public:
  ...
  void TrainModel(const T &mat, const std::vector<int> &labels) {
    // labels cannot be 0-9?? only support 0 or 1
...
private:
  ...
  // 做back propagation 需要原始数据值才可以 
  // 但是用我们现在的方法data representation 可能有点麻烦了吧
  // 或者开一个新的线程，在我们backpropagate的时候可以把这些值给搞出来即可
  ...
  const std::vector<std::vector<double>> *ForwardComputing(const T &mat) const {
    ...
    // int64_t layer_secs=0;
    // auto layer_start = std::chrono::system_clock::now();

    layer_connections_[0]->ComputeOutputs(mat, /*soft_max=*/false);

    // layer_secs = chrono::duration_cast<chrono::nanoseconds>(
    //    chrono::system_clock::now() - layer_start).count();
