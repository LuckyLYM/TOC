...
// T may be sparse_pair <int, double> here
...
public:
  ...
  bool insert_child(const T &key, TreeNode<T, F> *child) {
    // unordered_map. insert   return <iterator, bool>
    // *it it->first key value; it->second mapped value
...
private:
  // key value   F is the hash comparison function for Type T
  // children is an unordered_map
...
private:
  ...
  void InitRoot() {
    ...
    seq_num_ = 0;
    // first element for padding
    tree_nodes_.push_back(new TreeNode<T, F>());  // a TreeNode vector
  }
  ...
  // codes_ seems like the Table D in the original paper
...
template <typename T, typename F> bool LZWCompresser<T, F>::compress_data() {
  // intialize the root
  InitRoot();
  // store all unique <index,value> pair in init_data_ and tree_nodes_
  InitTree();

  const int num_rows = data_.size();

  // traverse all rows, build codes_ and tree_nodes_
  for (int i = 0; i < num_rows; i++) {
    std::vector<int> code_vec;
    int idx = 0;
    const int row_size = data_[i].size();

    // traverse the whole dimension
    while (idx < row_size) {
      int len = 0;
      ...
      // search the longest common path
